...
You own ALL (x,y) placement on the fixed path/water layout — place/move/remove/modify only, never new paths or water. Put attractions on the guest flow (entrance→exit), place rides next to water for excitement, and never reuse a tile that was already rejected.
