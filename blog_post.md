...
AI agents are already capable of a great deal on their own. We wanted to see how far that can go. We're looking to build an agent system that invents ideas, tests them, backs the ones that hold up, and then runs the business for the long term, judged on the one thing it can't fake: does it make money? The long game is bigger: a system that reinvests what it earns into new ventures and eventually runs the whole portfolio itself, all built on **neuro-san-studio**, our multi-agent framework. But a grand vision is easy to write down. Does it actually work?
...
You are handed an amusement park and one decision a day. Build a roller-coaster; hire a janitor; place a souvenir shop; pour cash into research. You commit a single move, the gates open, guests stream in, and by closing time the day's takings tell you how you did. Do that for a hundred days, one full episode, and earn as much reward as you can. That is **Mini Amusement Parks (MAPs)**, a business-simulation benchmark from Skyfall AI ([arXiv:2511.15830](https://arxiv.org/abs/2511.15830)), and it is genuinely hard: humans still beat the best LLM agents by roughly 10× on the **medium** difficulty we run.

The knotty part here is that only the cheapest attractions start unlocked, and everything stronger is gated behind research. That gating forces a tricky balance: research costs money up front and pushes the park into losses, but it unlocks the attractions that pay off later. Those early decisions compound over a hundred days, so a weak opening quietly caps how good the ending can get. In that way, it mirrors an actual business: every dollar a ride earns is capital to put back in, whether into a stronger ride, another shop, or more research. It's the same earn-and-reinvest loop behind our bigger vision, just scaled down to a single park. 
...
We don't point a single LLM at the game. The park is run by an agent network called `player`, built using AAOSA principle on neuro-san-studio, looping once per day across the hundred-day game. The diagram belows shows the network as a graph: the coordinator at the hub, the six specialists around it, and the coded tools they share.
...
So the result is three separate agent networks, each a mind with its own goal and its own timescale:
...
![The planner network](images/planner_network.png)
...
![The watcher network](images/watcher_network.png)
...
Getting there took real work up front: building the harness and engineering the prompts. But the seven episodes themselves ran hands-off: same models, same code, no human touching a prompt between them. The only thing that changed from one to the next was the memory the network wrote for itself. Here are the results.

![Cumulative reward across all seven episodes](images/all_attempts_trajectory.png)

Each line is one episode's cumulative reward trajectory, tracked day by day across the 100-day run, with the final totals listed in the legend. That's a **nearly 40× climb from the first episode to the seventh** (12,121 → 483,019), with no weight updates or human interaction. The biggest jumps come once the system locks in its core money-making strategy: build up rides, shops, and staff early, then spend on research to unlock higher-tier rides that pull in bigger crowds and lift the park's rating, neither hoarding cash nor over-investing but striking a balance, all written down where it can't forget it again. Below is a comparison of the 3rd and 7th episodes at days 10 through 99, the early one on the left and the best on the right:

![The early episode beside the best episode](images/run_comparison.png)
